...
# 2) Current portfolio value / Net worth (current value of invested money)
# 3) Realized profit (profit for sold stocks)
# 4) Total current profit = Realized profit + Unrealized Profit ( = current portfolio value - total invested amount).
# two tables - stock details and transactions.
# change price as price = price +- random()%(price/100) every 2-3 seconds to show live changes if possible (at a low latency).
